...
- When a mechanic permits different layouts, demonstrate more than one on the same deterministic timeline. Preview each new arrangement before the player responds, preserve full-body clearance, and let the final projectile row pass before repositioning. Derive the warning, actual opening, and player route from the same layout data.
