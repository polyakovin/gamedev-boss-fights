...
- Node suite: 23 tests covering charge commitment, body clearance, swept hit detection, recovery, the tank and monster illustrations, design-focused teaching, generic boss/player labels, safe markup, schema failures, translation parity, source-version freshness, draft acceptance, the community call to review facts, concepts, examples, and translations, required 2D/3D YouTube references, exact timestamps for videos longer than three minutes, and learning sources spanning design, animation, accessibility, and implementation.
- Browser suite: 11 tests covering all locale routes, the compact two-column lesson header, the stacked 375 px layout, six external boss-behavior references, seven learning sources, and the community review note per locale, removal of the quiz block, language switching, Arabic direction, both animation outcomes, keyboard playback/seeking, reduced-motion behavior, and content without JavaScript.
