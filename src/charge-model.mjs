...
/** The player moves sideways after lock and clears the entire body and lane. */
export function playerPosition(time, plan = DEFAULT_PLAN) {
  const t = clamp(Number.isFinite(time) ? time : 0, 0, DURATION);
  if (t < PHASE_ENDS[0]) {
...
export function chargeFrame(time, plan = DEFAULT_PLAN) {
  const t = clamp(Number.isFinite(time) ? time : 0, 0, DURATION);
  const phase = phaseAt(t);
  const player = playerPosition(t, plan);
