...
test('boss builder persists a local draft and downloads portable JSON', async ({ page }) => {
  ...
  await expect(page.locator('.boss-builder-mechanic--wip')).toHaveCount(7);
  ...
  await expect(page.locator('.boss-builder-mechanic [data-blueprint-preview]')).toHaveCount(111);
...
test('lens chips show explanations and open localized lens pages', async ({ page, request }) => {
  ...
  await expect(page.locator('.lens-mechanic-card')).toHaveCount(71);
  ...
  await expect(page.locator('.lens-mechanic-card h2 .icon--directional')).toHaveCount(71);
  await page.locator('.language-menu summary').click();
  const languageLinks = await page
    .locator('.language-menu nav a')
    .evaluateAll((links) => links.map((link) => link.getAttribute('href')));
...
test('the root defaults to English and localized catalogs point to real pages', async ({
  ...
  await expect(page.locator('.catalog-lesson--wip')).toHaveCount(7);
  await expect(page.locator('.catalog-lesson:not(.catalog-lesson--wip)')).toHaveCount(117);
  ...
  await expect(page.locator('.catalog-lesson__preview [data-blueprint-preview]')).toHaveCount(111);
