...
test('boss builder persists a local draft and downloads portable JSON', async ({ page }) => {
  ...
  await expect(page.locator('.boss-builder-mechanic--wip')).toHaveCount(26);
  await expect(page.locator('.boss-builder-mechanic:not(.boss-builder-mechanic--wip)')).toHaveCount(
    98,
  );
  await expect(page.locator('.boss-builder-mechanic [data-character-art="kern"]')).toHaveCount(98);
  await expect(page.locator('.boss-builder-mechanic [data-character-art="tavi"]')).toHaveCount(98);
  await expect(page.locator('.boss-builder-mechanic [data-blueprint-preview]')).toHaveCount(92);
...
test('the root defaults to English and localized catalogs point to real pages', async ({
  ...
  await expect(page.locator('.catalog-lesson--wip')).toHaveCount(26);
  await expect(page.locator('.catalog-lesson:not(.catalog-lesson--wip)')).toHaveCount(98);
  ...
  await expect(page.locator('.catalog-lesson__preview [data-blueprint-preview]')).toHaveCount(92);
  ...
  await expect(page.locator('.game-example')).toHaveCount(3);
  await expect(page.locator('.lens-chip')).toHaveCount(5);
  const draftPage = await request.get('en/mechanics/damage-rate-cap/');
