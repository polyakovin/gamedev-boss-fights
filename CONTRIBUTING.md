...
Published mechanics also include concrete 2D and 3D boss examples. Link to a focused YouTube clip of the behavior, preferably no longer than three minutes, and explain what it demonstrates for a designer. If only a longer useful video exists, its URL must open immediately before the mechanic with `&t=<seconds>s`. Record the full length in `videoDurationSeconds`; do not use a speedrun merely because it is short. Do not upload video frames or thumbnails to the repository, and do not turn the explanation into a walkthrough.
