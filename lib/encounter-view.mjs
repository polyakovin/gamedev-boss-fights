...
export function renderEncounterArena(id) {
  const key = escape(id);
  const stones = Array.from({ length: 14 }, (_, row) =>
    Array.from({ length: 6 }, (_, column) => {
      const x = column * 104 - (row % 2) * 52;
      const y = row * 68;
      return `<rect x="${x + 3}" y="${y + 3}" width="98" height="62" rx="5" fill="${(row + column) % 3 === 0 ? '#354443' : '#2d3b3b'}" stroke="#465350" stroke-opacity=".32" />`;
    }).join(''),
  ).join('');
  return /* HTML */ `<defs>
