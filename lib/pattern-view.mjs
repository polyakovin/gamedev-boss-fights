  ...
  >
    <div class="pattern-demo__canvas">
      ...
      >
        ...
        ${renderEncounterArena(id, { minimal: kind === 'sweep' || kind === 'summon' || kind === 'gap-volley' || kind === 'projectile-fan' })}${kind === 'sweep' ? sweepScene : ''}${kind === 'summon' ? summonScene : ''}${kind === 'gap-volley' ? gapVolleyScene : ''}${kind === 'projectile-fan' ? projectileFanScene : ''}${sweepLayer}${slamLayer}${summonLayer}${volleyLayer}${fanLayer}
...
export function renderPatternThumbnail(kind, id) {
  ...
  return /* HTML */ `<svg viewBox="0 0 360 160" data-pattern-preview="${escape(kind)}">
    ...
    <rect
      width="360"
      height="160"
      fill="${['sweep', 'gap-volley', 'projectile-fan'].includes(kind) ? '#263434' : `url(#pattern-preview-${escape(id)})`}"
    />
    ${layer}
