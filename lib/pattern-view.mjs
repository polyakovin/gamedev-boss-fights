import { patternDuration, patternFrame, slamWavePath } from '../src/pattern-model.mjs';
...
const sweepScene = /* HTML */ `<g data-pattern-sweep-scene aria-hidden="true">
  <path
  ...
  />
  <path
    d="M 56 117 H 129 V 348 H 56 Z M 431 117 H 504 V 348 H 431 Z M 145 119 H 415 V 153 H 145 Z M 70 546 L 280 515 L 490 546 V 581 L 280 548 L 70 581 Z M 72 758 L 280 722 L 488 758 V 792 L 280 758 L 72 792 Z"
    fill="var(--accent)"
    opacity=".23"
  />
  <path
    d="M 213 340 L 280 320 L 347 340 L 360 390 L 280 417 L 200 390 Z M 80 836 L 280 802 L 480 836 V 852 L 280 819 L 80 852 Z"
    fill="var(--muted)"
    opacity=".3"
  />
</g>`;
const slamScene = /* HTML */ `<g data-pattern-slam-scene aria-hidden="true">
  <path
    d="M 38 94 H 522 V 878 H 38 Z M 58 280 H 502 V 878 H 58 Z"
    fill="var(--muted)"
    opacity=".2"
  ...
  />
  <path
    d="M 198 303 L 280 278 L 362 303 L 373 345 L 280 375 L 187 345 Z M 80 836 L 280 802 L 480 836 V 852 L 280 819 L 80 852 Z"
    fill="var(--muted)"
    opacity=".3"
  />
  <path
    d="M 255 348 L 229 376 L 192 382 M 304 352 L 335 376 L 369 380 M 219 541 L 195 566 L 150 575 M 358 534 L 393 558 L 434 568 M 246 750 L 211 776 L 168 784 M 316 748 L 355 771 L 398 782"
    fill="none"
    stroke="var(--muted)"
    stroke-width="7"
    opacity=".18"
...
const projectileFanScene = /* HTML */ `<g data-pattern-fan-scene aria-hidden="true">
  <path
    d="M 38 94 H 522 V 878 H 38 Z M 58 280 H 502 V 878 H 58 Z"
    fill="var(--muted)"
    opacity=".2"
  />
  <path
    d="M 56 117 H 129 V 348 H 56 Z M 431 117 H 504 V 348 H 431 Z M 145 119 H 415 V 153 H 145 Z M 70 494 L 280 464 L 490 494 V 528 L 280 497 L 70 528 Z M 72 748 L 280 713 L 488 748 V 782 L 280 749 L 72 782 Z"
    fill="var(--accent)"
    opacity=".23"
  />
  <path
    d="M 220 167 L 280 148 L 340 167 L 350 221 L 280 245 L 210 221 Z M 80 832 L 280 799 L 480 832 V 850 L 280 817 L 80 850 Z"
    fill="var(--muted)"
    opacity=".3"
  />
</g>`;
const slamLayer = /* HTML */ `<g data-pattern-slam opacity="0">
  <path data-pattern-slam-ring d="${slamWavePath(58)}" fill="var(--signal)" fill-opacity=".58" />
</g>`;
const summonLayer = /* HTML */ `<g data-pattern-summon opacity="0">
  <g
    ...
    stroke-width="2"
  >
    <ellipse cx="135" cy="465" rx="38" ry="14" /><ellipse
      cx="280"
      cy="515"
      rx="38"
      ry="14"
    /><ellipse cx="425" cy="465" rx="38" ry="14" />
  </g>
  ...
  >
    <div class="pattern-demo__canvas">
      ...
      >
        ...
        ${renderEncounterArena(id, { minimal: kind === 'sweep' || kind === 'ground-slam' || kind === 'projectile-fan', quiet: kind === 'summon' || kind === 'gap-volley' })}${kind === 'sweep' ? sweepScene : ''}${kind === 'ground-slam' ? slamScene : ''}${kind === 'projectile-fan' ? projectileFanScene : ''}${sweepLayer}${slamLayer}${summonLayer}${volleyLayer}${fanLayer}
...
export function renderPatternThumbnail(kind, id) {
  const layer =
    kind === 'sweep'
      ...
      : kind === 'ground-slam'
        ? `<g transform="translate(105 80) scale(.55) translate(-280 -310)"><path d="${slamWavePath(112)}" fill="var(--signal)" fill-opacity=".52" /></g>`
        : kind === 'summon'
  ...
  return /* HTML */ `<svg viewBox="0 0 360 160" data-pattern-preview="${escape(kind)}">
    ...
    <rect
      width="360"
      height="160"
      fill="${kind === 'summon' || kind === 'gap-volley' ? '#203330' : ['sweep', 'ground-slam', 'projectile-fan'].includes(kind) ? '#263434' : `url(#pattern-preview-${escape(id)})`}"
    />
    ${kind === 'ground-slam' ? '<path d="M 0 52 L 180 40 L 360 52 V 111 L 180 99 L 0 111 Z" fill="var(--muted)" opacity=".2" />' : ''}
    ${layer}
