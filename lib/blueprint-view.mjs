...
export function blueprintPreviewLayout(mechanicId) {
  ...
  return Object.freeze({
    ...
    allies:
      (frame.stackDamageAllies ?? frame.personalSpreadPositions?.slice(1))?.map((ally) =>
        previewPoint(ally, camera),
      ) ?? null,
...
export function renderBlueprint(demo, mechanicId) {
  ...
  const staticScene = QUIET_SCENES.has(mechanicId)
    ? []
    : (STATIC_SCENE_BACKDROPS[mechanicId] ?? []);
  ...
  >
    <div class="blueprint-demo__canvas">
      ...
      >
        ...
        ${
          mechanicId === 'stack-damage' || mechanicId === 'personal-spread'
            ? (initial.stackDamageAllies ?? initial.personalSpreadPositions.slice(1))
                .map(
                  (ally, index) =>
