...
export async function loadContent() {
  ...
  const [
    ...
    readJson('content/mechanics-index-locales/en.json'),
...
export async function validateContent(data) {
  ...
  const publishedIds = new Set(
    data.mechanics.filter(({ meta }) => meta.published).map(({ meta }) => meta.id),
  );
  for (const id of popularIds) {
    const hasExactlyOneDetailedPage = publishedIds.has(id) !== popularProfileIds.has(id);
    if (!hasExactlyOneDetailedPage)
      errors.push(
        `content/popular-mechanics: ${id} must be either published or have one draft profile`,
      );
  }
