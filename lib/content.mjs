...
export async function loadContent() {
  ...
  const [mechanicsIndex, mechanics, lenses] = await Promise.all([
    readJson('content/mechanics-index.json'),
    loadCollection('mechanics', locales),
    loadCollection('lenses', locales),
  ]);
  return { locales, ui, mechanicsIndex, mechanics, lenses };
}
export async function validateContent(data) {
  ...
  const validators = {};
  for (const name of ['meta', 'lesson', 'lens-meta', 'lens', 'ui', 'mechanics-index'])
