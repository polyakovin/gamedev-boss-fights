import { DURATION } from '../src/charge-model.mjs';
  ...
  >
    <div class="charge-demo__canvas">
      <div class="charge-demo__scene-timeline">
        <div class="charge-demo__phase-labels" role="list" aria-label="${escape(demo.step)}">
          ${demo.phaseNames.map((name, index) => `<span class="charge-demo__phase-label" data-charge-phase-label="${index}" role="listitem" tabindex="0" aria-describedby="charge-phase-tooltip-${index}">${escape(name)}<span class="charge-demo__phase-tooltip" id="charge-phase-tooltip-${index}" role="tooltip">${escape(demo.phaseDescriptions[index])}</span></span>`).join('')}
        </div>
        <label class="charge-demo__timeline"
          ...
          ><input
            ...
            step="10"
            value="0"
          /><output data-charge-time dir="ltr">0.0 / ${DURATION.toFixed(1)}</output></label
        >
      </div>
      <svg
      ...
      >
        ...
        />
        <rect x="19" y="19" width="522" height="662" rx="21" fill="url(#charge-grid)" />
        <path
          d="M 46 46 h 36 M 46 46 v 24 M 514 46 h -36 M 514 46 v 24 M 46 654 h 36 M 46 654 v -24 M 514 654 h -36 M 514 654 v -24"
          fill="none"
        ...
        <g data-charge-lane transform="translate(280 155) rotate(90)">
          ...
          />
          <rect x="0" y="-49" width="390" height="98" rx="18" fill="url(#charge-hatch)" />
          <path
            d="M 65 0 H 365 M 346 -11 L 365 0 L 346 11"
            fill="none"
            stroke="var(--signal)"
            stroke-width="2.5"
        ...
        <path
          data-charge-dodge
          d="M 280 350 H 154 M 165 342 L 154 350 L 165 358"
          fill="none"
          stroke="var(--accent)"
          stroke-width="3"
        ...
        <g data-charge-player transform="translate(280 350)">${CHARGE_ART.monster}</g>
        <text
